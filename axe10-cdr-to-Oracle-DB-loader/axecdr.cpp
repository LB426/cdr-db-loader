...
void removeFfromBnum(unsigned char * bnum){
  int last = strlen((char*)bnum) - 1;
  if( 'F' == bnum[last]){
    //printf("bnum=%s\n",bnum);
    bnum[last] = 0;
  }
...
void FillRecordTypeMy(RECORD_TYPE00 *pRec, RECORD_TYPE00MY *pRecMy) {
  ...
  removeFfromBnum(pRecMy->n21_Bnumber);
...
void PrintRec(RECORD_TYPE00MY *pRec) {
  char n37[12];
  n37[11] = 0;
  strcpy(n37, (char*) pRec->n37_DateForStartOfChargingUSA);
  InSQLDataFormat(n37);
  char n40[12];
  n40[11] = 0;
  strcpy(n40, (char*) pRec->n40_TimeForStartOfCharging24hour);
  InSQLTimeFormat(n40);
  char n43[12];
  n43[11] = 0;
  strcpy(n43, (char*) pRec->n43_TimeForStopOfCharging);
  InSQLTimeFormat(n43);
  char n45[12];
  n45[11] = 0;
  strcpy(n45, (char*) pRec->n45_ChargeableDuration);
  DurSec(n45);
  ...
  if(globalArgs.verbose == 1){
    fprintf(globalArgs.dstFile, "%s %s %s %s %s %s %s %s %s\n", 
            pRec->n14_Anumber, pRec->n21_Bnumber, n37, n40, n43, n45, 
            pRec->n78_OutgouingRoute, pRec->n80_IncomingRoute, 
            globalArgs.srcFilenameBasename);
  }
  
  ocilib::ostring recordType = "00";
  ocilib::ostring Anum = (char*) pRec->n14_Anumber; 
  ocilib::ostring Bnum = (char*) pRec->n21_Bnumber;
  unsigned long Durt = std::atol((char*) n45);
  ocilib::ostring outRoute = (char*) pRec->n78_OutgouingRoute;
  ocilib::ostring incRoute = (char*) pRec->n80_IncomingRoute;
  ocilib::ostring start = n37;
  start.append(" "); 
  start.append(n40);
  
  aNmbrs.push_back(Anum);
  bNmbrs.push_back(Bnum);
  outRts.push_back(outRoute);
  incRts.push_back(incRoute);
  durs.push_back(Durt);
  begTms.push_back(start);
  recTypes.push_back(recordType);
}
...
void PrintRec(RECORD_TYPE04MY *pRec) {
  ...
  if(globalArgs.verbose == 1){
    fprintf(globalArgs.dstFile, "%s %s %s %s %s %s %s %s %s\n", pRec->n14_Anumber,
            pRec->n21_Bnumber, n37, n40, n43, n45, 
            pRec->n78_OutgouingRoute, pRec->n80_IncomingRoute, 
            globalArgs.srcFilenameBasename);
  }
  ocilib::ostring recordType = "04";
  ocilib::ostring Anum = (char*) pRec->n14_Anumber; 
  ocilib::ostring Bnum = (char*) pRec->n21_Bnumber;
  unsigned long Durt = std::atol((char*) n45);
  ocilib::ostring outRoute = (char*) pRec->n78_OutgouingRoute;
...
void PrintRec(RECORD_TYPE06MY *pRec) {
  ...
  if(globalArgs.verbose == 1){
    fprintf(globalArgs.dstFile, "%s %s %s %s %s undefined undefined undefined %s\n", 
            pRec->n14_Anumber, pRec->n21_Bnumber, n37, n40, n43, 
            globalArgs.srcFilenameBasename);
  }
  ocilib::ostring recordType = "06";
  ocilib::ostring Anum = (char*) pRec->n14_Anumber; 
  ocilib::ostring Bnum = (char*) pRec->n21_Bnumber;
  unsigned long Durt = 0;
  ocilib::ostring outRoute = "undef";
  ocilib::ostring incRoute = "undef";
  ocilib::ostring start = n37;
  start.append(" "); 
  start.append(n40);
...
void PrintRec(RECORD_TYPE07MY *pRec) {
  ...
  RejectAllSpaces((char*) pRec->n14_Anumber);
  if(globalArgs.verbose == 1){
    fprintf(globalArgs.dstFile, 
            "%s undefined %s %s undefined undefined undefined undefined %s\n", 
            pRec->n14_Anumber, n37, n40, globalArgs.srcFilenameBasename);
  }
  ocilib::ostring recordType = "07";
  ocilib::ostring Anum = (char*) pRec->n14_Anumber; 
  ocilib::ostring Bnum = "undef";
  unsigned long Durt = 0;
  ocilib::ostring outRoute = "undef";
  ocilib::ostring incRoute = "undef";
  ocilib::ostring start = n37;
...
void PrintRec(RECORD_TYPE08MY *pRec) {
  ...
  if(globalArgs.verbose == 1){
    fprintf(globalArgs.dstFile, 
            "%s undefined undefined undefined undefined undefined undefined undefined %s\n", 
            pRec->n14_Anumber, globalArgs.srcFilenameBasename);
  }
  ocilib::ostring recordType = "08";
  ocilib::ostring Anum = (char*) pRec->n14_Anumber; 
  ocilib::ostring Bnum = "undef";
  unsigned long Durt = 0;
  ocilib::ostring outRoute = "undef";
  ocilib::ostring incRoute = "undef";
  ocilib::ostring start = InSQLDataFormat((char*)pRec->n112_DateOfExecutionOfCommand);
  start.append(" "); 
  start.append( InSQLTimeFormat((char*)pRec->n113_TimeOfExecutionOfComand) );
  aNmbrs.push_back(Anum);
  bNmbrs.push_back(Bnum);
  outRts.push_back(outRoute);
  incRts.push_back(incRoute);
...
void GetRecordsFromBuf(char * buffer) {
  ...
  while ((x = (pBuffer - buffer)) < 2048) {
    ...
    if (memcmp(rectype, NE, sizeof (NE)) == 0) {
      ...
      PrintRec(&RecType09My);
      pBuffer = pBuffer + sizeof (RECORD_TYPE09);
      continue;
    } else if (*pBuffer == 0) {
      pBuffer++;
    } else {
      fprintf(globalArgs.dstFile, "GetRecordsFromBuf() - Unknown Record Type: %c%c\n", *pBuffer, *(pBuffer + 1));
      break;
    }
  }
}
...
void prepareDataBatchLoading(){
  ...
  if ((globalArgs.srcFile = fopen(globalArgs.srcFileName, "rb")) == NULL) {
    printf("prepareDataBatchLoading()- Error open file for read: \"%s\"\n", globalArgs.srcFileName);
    exit(1);
  }
  if (globalArgs.verbose == 1)
