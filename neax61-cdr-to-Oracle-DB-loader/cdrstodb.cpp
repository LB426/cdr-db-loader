...
int checkfileinbase(){
  ...
  while (rs1++)                                                         
  {
    ...
    //std::cout << "id=" << id << " md5=" << md5sum << std::endl;
    count++;
  }
...
void insert_data_to_table() {
  ...
  Statement st(con);
  st.Prepare("INSERT INTO neax61 "
             "(anum,bnum,chargstart,dur,incrt,outrt,loadfilelog_id) "
             "VALUES (:anum,:bnum,:chargstart,:dur,:incrt,:outrt,:fid)");
  ...
  if (globalArgs.verbose == 1) {
    std::cout << "neax61 row processed : " << st.GetAffectedRows() << std::endl;
  }
}
...
int loadCDRsToOraDb() {
  ...
  try {
    ...
    std::cout << "EXCEPTION in loadCDRsToOraDb: " << ex.what() << std::endl;
    err = -1;
  }
  Environment::Cleanup();
  return err;
