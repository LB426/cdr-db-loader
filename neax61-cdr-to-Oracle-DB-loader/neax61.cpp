...
void PrintRec(AMA_FORMAT_MY *pAmaRecMy) {
  ...
  if(globalArgs.verbose == 1){
    fprintf(globalArgs.dstFile, "%s %s '%s %s' %s %s %s\n", anomb, bnomb, date, time, durat, incr, outr);
  }
  
  ocilib::ostring Anum = anomb; 
  ocilib::ostring Bnum = bnomb;
  unsigned long Durt = std::atol((char*) durat);
  ocilib::ostring outRoute = (char*) outr;
  ocilib::ostring incRoute = (char*) incr;
  ocilib::ostring start = date;
  start.append(" "); 
  start.append(time);
